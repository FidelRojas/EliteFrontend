...
import TruckForm from "../components/Trurck/TruckForm"
...
export const TrucksPage = () => {
  ...
  const handleCreateTruck = async (newTruck: Truck) => {
    try {
      ...
      } = await client.post(TRUNK_PATH, newTruck)
  ...
  const handleUpdateTruck = async (updateTruck: Truck) => {
    try {
      const {
        data: { message, data: truck },
      } = await client.put(`${TRUNK_PATH}/${updateTruck?.id}`, updateTruck)
